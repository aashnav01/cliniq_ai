        ...
        <div className="panel">
          ...
          <div className="form-grid">
            ...
            <div className="form-field full" style={{ position: 'relative' }}>
              ...
              <input type="text" value={isListening ? form.symptoms + (form.symptoms && !form.symptoms.endsWith(' ') ? ' ' : '') + transcript : form.symptoms} onChange={e => setForm({ ...form, symptoms: e.target.value })} placeholder="e.g. chest pain, dyspnoea, diaphoresis" style={{ paddingRight: '40px' }} />
              <button 
                onClick={toggleListening} 
                style={{ 
                  position: 'absolute', bottom: '6px', right: '6px', 
                  background: isListening ? 'var(--critical)' : 'transparent', 
                  color: isListening ? '#fff' : 'var(--text-muted)', 
                  border: 'none', 
                  borderRadius: '50%', width: '28px', height: '28px', 
                  display: 'flex', alignItems: 'center', justifyContent: 'center', 
                  cursor: 'pointer', transition: 'all 0.2s', boxShadow: isListening ? '0 0 10px rgba(255, 107, 107, 0.5)' : 'none'
                }}
