          ...
          <div className="output-area">
            {!output ? (
              ...
              <div>
                ...
                {/* NEWS2 Score */}
                {output.data?.news2 && (
                  <div className={`news2-card news2-${output.data.news2.color}`}>
                    <div className="news2-header">
                      <span className="news2-label">NEWS2 Score</span>
                      <span className="news2-score">{output.data.news2.score} / 20</span>
                      <span className={`news2-severity news2-badge-${output.data.news2.color}`}>
                        {output.data.news2.severity?.toUpperCase()}
                      </span>
                    </div>
                    <p className="news2-action">{output.data.news2.action}</p>
                    {Object.keys(output.data.news2.breakdown || {}).length > 0 && (
                      <div className="news2-breakdown">
                        {Object.entries(output.data.news2.breakdown).map(([k, v]) => (
                          <span key={k} className="news2-chip">
                            {k.replace('_', ' ')}: {v.value ?? v.points !== undefined ? `${v.points}pt` : '—'}
                          </span>
                        ))}
                      </div>
                    )}
                    {output.data.qsofa && (
                      <div className={`qsofa-row ${output.data.qsofa.positive ? 'qsofa-positive' : ''}`}>
                        <span>qSOFA: {output.data.qsofa.score}/3</span>
                        <span>— {output.data.qsofa.interpretation}</span>
                      </div>
                    )}
                  </div>
                )}
                ...
                {/* ICD-10 Suggestions */}
                {output.data?.icd10?.suggestions?.length > 0 && (
                  <div className="icd10-section">
                    <div className="icd10-header">🏷 ICD-10 Billing Codes</div>
                    <div className="icd10-strip">
                      {output.data.icd10.suggestions.map((c, ci) => (
